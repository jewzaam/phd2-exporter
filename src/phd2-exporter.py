...
def handleEvent(s, data):
    global PHD_STATE, PHD_RMS_SAMPLES, GLOBAL_LABELS, PHD_RMS_DATA
    ...
    elif event in ["GuideStep", "SettleDone"]:
        PHD_STATE="Guiding"
    ...
    # did equipment change? get pixel scale!
    ...
    if event in ["LockPositionSet", "StarSelected"]:
        ...
        if calculate_rms == True:
            ...
            for key in PHD_RMS_DATA.keys():
                if key not in rms_square_of_sums:
                    rms_square_of_sums[key] = 0
                # do the sum of squares bit
                for value in PHD_RMS_DATA[key]:
                    rms_square_of_sums[key] += math.pow(value, 2)
                # then divide and sqrt
                rms_px[key] = math.sqrt(rms_square_of_sums[key] / PHD_RMS_SAMPLES)
                utility.set("phd2_rms", rms_px[key], {"source": key, "scale": "px"})
            
                # calculate for arcsec if we have pixel scale
                if PIXEL_SCALE > 0:
                    rms_arcsec = rms_px[key] * PIXEL_SCALE
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                while True:
                    try:
                        ...
                        # this is fine, just nothing to read.  PHD2 idle.
                        continue
